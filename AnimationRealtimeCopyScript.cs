...
public class AnimationRealtimeCopyScript : MonoBehaviour
{
    public UdpReceiveScript udpReceive;
    ...
    void Update()
    {
        string receivedData = udpReceive.ReceivedData;
        ...
        if (points.Length >= 99) // 檢查陣列長度是否足夠大
        {
            for (int i = 0; i < 33 && i < Body.Length; i++)
            {
                ...
                if (float.TryParse(points[0 + (i * 3)], out x) && float.TryParse(points[1 + (i * 3)], out y) && float.TryParse(points[2 + (i * 3)], out z))
                {
                    ...
                    y /= 100f;
                    z /= 300f;
                    Vector3 targetPosition = new Vector3(x, y + 8, z);
    ...
    {
        yield return new WaitForSeconds(0.03f); // 等待 0.03 秒
    }
}
